...
uint32_t testHaD() {
    ...
    for (int i = 0; i < 0x10; i++) {
        tft.setAddrWindow(0, 0, MIN(240, tft.width()), MIN(320, tft.height()));
        ...
        while (cmp < &HaD_240x320[sizeof(HaD_240x320)]) {
            ...
            cnt = MIN(cnt, (uint16_t)tft.width());
...
#endif
    tft.setTextColor(ST7735_WHITE);
    tft.print(F("SPI LCD on 328p@16MHz"));
